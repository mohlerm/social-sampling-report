...
Our goal is to achieve a good estimation of $\bar{f}$ using as little samples $S$ as possible with respect to an additive error bound $\epsilon$.
...
Therefore we can analyze our sampler for estimator properties like bias and we will might also denote our samplers as estimators.
...
It can be shown that the second largest eigenvalue of $L^2$ equals $\lambda_2^2 = ||L^2 - \frac{D^{1/2}\mathds{1}\mathds{1}^TD^{1/2}}{2m}||$ and therefore $1-\lambda_2$ is the second smallest eigenvalue of the normalized Laplacian. The proof follows.
\end{proof}
...
We are aware this contradicts the initial definition of a sampler but we since $k$ has no usage in the other samplers we keep the original definition anyways while still calling the \texttt{Sparse} estimator a sampler.
